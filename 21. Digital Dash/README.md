...
This project is a port of the original Arduino sketch (see `docs/digital_dash_amega328p_port_prompt.md`) with three additions: an EEPROM-persisted high score shown on the splash screen and game-over screen, a proper RNG seed from a floating ADC pin (the original always started with the same sequence), and a non-blocking architecture.
