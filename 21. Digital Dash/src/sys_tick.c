...
/* Incremented by ISR every 1 ms. Declared volatile because it is written
 * from an ISR and read from main context. */
...
{
    uint32_t t;
    /* ATOMIC_BLOCK prevents a torn read of the 4-byte counter on an 8-bit CPU.
     * ATOMIC_RESTORESTATE preserves the I flag so this is safe both before
     * and after sei().  (avr-libc util/atomic.h) */
