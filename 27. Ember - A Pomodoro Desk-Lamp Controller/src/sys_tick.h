 ...
 * sys_tick.h
 *
 * 1 ms system tick via Timer0 CTC. Every timed module in Ember (button
 * debounce, buzzer note timing, and the Pomodoro countdown) schedules
 * itself off sys_millis()/sys_elapsed() instead of blocking _delay_* calls,
 * so this must be initialised before anything else that needs timing.
 *
 * Copied from relay/sentinel/src/sys_tick.{c,h} (guard/comments renamed only).
 * ATmega328P datasheet §15 (Timer0/Counter0)
 */

#ifndef EMBER_SYS_TICK_H_
#define EMBER_SYS_TICK_H_
