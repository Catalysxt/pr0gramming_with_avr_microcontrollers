 ...
 * comfortably outlasts contact bounce (typically < 5 ms) without adding felt lag. */
#define BUTTON_DEBOUNCE_MS   15u

/* Hold time that promotes a press to a long-press (~1 s per the UI spec). Capped
 * by held_ms's uint16_t range, which we never approach. */
