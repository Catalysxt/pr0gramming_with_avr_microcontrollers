 ...
 *
 * Sleep interaction: sleep.c calls buzzer_off() before power_timer2_disable()
 * — buzzer_off() zeroes TCCR2A/B and tristates the pin, which avoids OC2A
 * freezing high mid-tone and biasing the piezo for the whole sleep duration.
 */
...
{
    BUZZER_DDR |= (1u << BUZZER_BIT);  /* PB3 as output for OC2A, datasheet §17.6.2 */
}
...
/* ---- SFX library — one per key class (Ascent spec, distinct from Anvil's
 * PIN-safe set) ---- */
const note_t SFX_DIGIT[1] PROGMEM = {
    { 2000u, 20u },
};

const note_t SFX_OPERATOR[1] PROGMEM = {
    { 1500u, 30u },
};

const note_t SFX_EQUALS[3] PROGMEM = {
    { 1000u, 70u },
    { 1500u, 70u },
    { 2000u, 70u },
};

const note_t SFX_CLEAR[2] PROGMEM = {
    { 2000u, 50u },
    {  800u, 50u },
};

const note_t SFX_ERROR[1] PROGMEM = {
    { 200u, 400u },
};
