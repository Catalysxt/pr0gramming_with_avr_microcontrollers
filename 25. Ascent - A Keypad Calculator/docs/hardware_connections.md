...
| LED | **PB4** | OUT | Push-pull | Connected, but unused by v1 firmware — reserved for future use |
| Keypad COL0 | **PC0** | IN | Internal pull-up, PCINT8 enabled for sleep-wake |  |
| Keypad COL1 | **PC1** | IN | Internal pull-up, PCINT9 enabled for sleep-wake |  |
| Keypad COL2 | **PC2** | IN | Internal pull-up, PCINT10 enabled for sleep-wake |  |
| Keypad COL3 | **PC3** | IN | Internal pull-up, PCINT11 enabled for sleep-wake |  |
| Keypad ROW0 | **PC4** | OUT | High during scan; driven LOW before sleep |  |
| Keypad ROW1 | **PC5** | OUT | High during scan; driven LOW before sleep |  |
| Keypad ROW2 | **PD2** | OUT | High during scan; driven LOW before sleep |  |
| Keypad ROW3 | **PD3** | OUT | High during scan; driven LOW before sleep |  |
...
Reserved free pins for stretch work: PD0 (RXD), PD1 (TXD) — a USART
debug log is an explicit stretch goal. PC7 does not exist on the DIP-28
package.
...
| GND | Ground rail | LCD BLK (pin 16) | — | Optional 220 Ω series for backlight |
| GND | Ground rail | Pot terminal 3 | — |  |
| VEE | Pot wiper | LCD VO (pin 3) | — | Adjust until chars visible |
...
| BUZZER | Piezo (−) | GND | — |  |
| LED | PB4 | LED anode | — | Wired but unused by v1 firmware |
| LED | LED cathode | GND | 330 Ω |  |
| KEY_COL0 | PC0 | Keypad COL0 pin | — | Internal pull-up + PCINT8 |
| KEY_COL1 | PC1 | Keypad COL1 pin | — | Internal pull-up + PCINT9 |
| KEY_COL2 | PC2 | Keypad COL2 pin | — | Internal pull-up + PCINT10 |
| KEY_COL3 | PC3 | Keypad COL3 pin | — | Internal pull-up + PCINT11 |
| KEY_ROW0 | PC4 | Keypad ROW0 pin | — | Idle high, LOW during sleep |
| KEY_ROW1 | PC5 | Keypad ROW1 pin | — | Idle high, LOW during sleep |
| KEY_ROW2 | PD2 | Keypad ROW2 pin | — | Idle high, LOW during sleep |
| KEY_ROW3 | PD3 | Keypad ROW3 pin | — | Idle high, LOW during sleep |
...
ROW3 ───┤ PD3                │
        │                PC0 ├──── KEY COL0 (PCINT8)
        │                PC1 ├──── KEY COL1 (PCINT9)
        │                PC2 ├──── KEY COL2 (PCINT10)
        │                PC3 ├──── KEY COL3 (PCINT11)
