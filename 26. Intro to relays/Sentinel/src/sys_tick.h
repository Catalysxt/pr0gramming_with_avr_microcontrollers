 ...
 * sys_tick.h
 *
 * 1 ms system tick via Timer0 CTC. Every timed module in this project
 * (button debounce, AUTO-mode relay toggle) schedules itself off
 * sys_millis()/sys_elapsed() instead of blocking _delay_* calls, so this
 * must be initialised before anything else that needs timing.
 *
 * Copied from keypad/anvil/src/sys_tick.{c,h} (guard/comments renamed only).
 * ATmega328P datasheet §15 (Timer0/Counter0)
 */

#ifndef SENTINEL_SYS_TICK_H_
#define SENTINEL_SYS_TICK_H_
